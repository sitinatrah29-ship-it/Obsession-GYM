  ...
  return h('div', { className: "max-w-7xl mx-auto px-4 sm:px-6 lg:px-8" },
    h('div', { className: "grid lg:grid-cols-2 gap-16 items-center" },
      h('div', { className: "relative group" },
        h('div', { className: "absolute -inset-4 bg-brand-primary/20 rounded-lg blur-2xl group-hover:bg-brand-primary/30 transition-all duration-500" }),
        h('img', { 
      ...
      h('div', null,
        h('span', { className: "text-brand-primary font-bold uppercase tracking-[0.3em] text-sm block mb-4" }, "The Facility"),
        h('h2', { className: "text-4xl md:text-5xl font-black font-display text-white mb-8 italic uppercase leading-tight" },
          "A Powerhouse ", h('br'), "For High Performers"
        ),
        h('p', { className: "text-lg text-gray-400 mb-8 leading-relaxed" },
        ...
        h('ul', { className: "space-y-4" },
          listItems.map((item, idx) => 
            h('li', { key: idx, className: "flex items-center text-gray-300" },
              h('span', { className: "w-5 h-5 bg-brand-primary/20 flex items-center justify-center rounded-full mr-4" },
                h('div', { className: "w-2 h-2 bg-brand-primary rounded-full" })
              ),
              h('span', { className: "font-semibold uppercase tracking-wider text-sm" }, item)
            )
          )
        )
      )
